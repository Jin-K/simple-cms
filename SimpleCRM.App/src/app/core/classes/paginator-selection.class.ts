...
import { ItemsSelectionState }    from '../enums/items-selection-state.enum';
import { PaginationItemList }     from '../models/pagination-items-list.type';

export class PaginatorSelection<T> extends SelectionModel<T> {
  ...
  constructor(private dataAndCountSubject: Subject<PaginationItemList>) {
    super(true, []);
  ...
  private onPaginated(paginatedItemsList: PaginationItemList) {
